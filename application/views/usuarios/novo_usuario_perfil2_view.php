...
<div class="painel_novo_user">
    ...
    <div class="text_explicativo"> 
        ...
        Informe seus dados › Cadastro de condomínio
    </div>
    <form id="form_novo_usuario" method="post" action="<?= base_url('usuarios/insert_perfil2') ?>">
        <div class="form-group">
            <div class="input-group">
                <div class="input-group-addon">Nome do condomínio</div>
                <input id="textNome" name="nome" value="<?= $inputs['nome'] ?>" class="form-control" placeholder=" Digite o nome do condomínio" type="text">
            </div>
        </div>
        <div class="form-group">
            <div class="input-group">
        ...
        <div class="form-group">
            <div class="input-group">
                Você é uma revenda?
                <?php
                    if(!isset($inputs['perfil_revenda'])){
                        $perfil_revenda = "0";
                    }else{
                        $perfil_revenda = $inputs['perfil_revenda'];
                    }
                    if($perfil_revenda == "1"){
                    
                    ?>
                        <input onchange=view_alert_revenda(this.value) name="perfil_revenda" value="0" id="perfil_revenda_no" type="radio"><label for="perfil_revenda_no">Não</label>
                        &nbsp;
                        <input onchange=view_alert_revenda(this.value) name="perfil_revenda" value="1" id="perfil_revenda_yes" checked="true" type="radio"><label for="perfil_revenda_yes">Sim</label>
                    <?php
                    }else{
                    ?>
                        <input onchange=view_alert_revenda(this.value) name="perfil_revenda" value="0" id="perfil_revenda_no" checked="true" type="radio"><label for="perfil_revenda_no">Não</label>
                        &nbsp;
                        <input onchange=view_alert_revenda(this.value) name="perfil_revenda" value="1" id="perfil_revenda_yes" type="radio"><label for="perfil_revenda_yes">Sim</label>
                    <?php
                    }
                ?>
            </div>
        </div>
        <div id="info_revenda" class="alert alert-info" role="alert">
            <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
            Informe seu e-mail e senha da sua conta de revenda.
        </div>
        <div class="form-group">
            <div class="input-group">
...
<script>
    ...
    view_alert_revenda(<?=$perfil_revenda?>);
</script>